...
auto_split <- function(in_fastq, size = 284289, prefix = "split_",
                       threads = 1, out_dir = paste0(dirname(in_fastq), 
                       "/split"), extension = NULL, target_ram_size_gb = 10) {
  # Print progress
  ...
  if (stats$number_files > 1) {
    ...
    # Print message
    cat("\nOnly one file would be created, no need to split moving file to out_dir\n")
    # In this case, just copy the original file to the out_dir
    if (!dir.exists(out_dir)) {
      dir.create(out_dir, showWarnings = FALSE)
    }

    # Assign command
    cmd <- paste0("cp") # Use cp to copy the original file
    # Construct the full command to copy the file
    args <- c(in_fastq, paste0(" ", out_dir))

    # Run command
    system2(command = cmd, 
             args = args,
             stdout = TRUE,
             stderr = TRUE) # Capture output and errors
    # Check if the copy was successful
    if (file.exists(paste0(out_dir, "/", basename(in_fastq)))) {
      cat("\nSuccessfully copied the original file to the output directory.\n")
    } else {
      stop("\nFailed to copy the original file to the output directory. Please check permissions.\n")
    }
    
    # Update stats to reflect the single file
    stats$number_files <- 1 # Set to 1 since we did not split
    stats$number_of_reads_per_file <- stats$reads # since no split, it is the same as original reads
  }
