---
title: "TLD Example Vignette HiFi Data"
author: "Jake Reed"
date: '`r Sys.Date()`'
output:
  html_document: 
...
# Sample name
sample_name <- "HG002_HiFi"

# Create results directory
dir.create(file.path(paths$example_results, sample_name), 
           showWarnings = FALSE)

sample_file <- file.path("/media/jake/Backup_2405/backup_2405/telolrs/data/shared_data/hs/data/SRX143_HG002/SRR182_HiFi.fastq")
...
motif_analysis <- telomere_motif(data_ref = reference,
                                 ...
                                 telo_motif_length = 6, 
                                 number_of_motifs = 10,
                                 number_of_repeats = 15, # For HiFi
                                 #number_of_repeats = 15 # For ONT 
                                 platform = "ONT",
                                 threads = 18)

cat("Number of motifs found: ", length(motif_analysis$motifs))
cat("\n")
cat("Grep pattern for pattern matching and initial search: ", 
...
```{r inspect_threshold_data, eval = FALSE}
# Load data
telomere_frequencies <- readRDS(file.path(paths$example_results,
                                          sample_name,
                                          "motif_frequencies.rds"))
...
ind_2k <- which(telomere_lengths < 2000 & 
                ...
                telomere_lengths > 0)
cat("Number of telomeres less than 2kbp: ", length(ind_2k), "\n")
...
## Plot threshold data
```{r plot_threshold_data, eval = FALSE}
library(ggplot2)
library(cowplot)
...
# Print fraction na
cat("Fraction NA: ", sum(is.na(results_df$telomere_end)) / nrow(results_df))

# Telomere update print(paste0)
cat("Number of telomeres found: ", length(telomere_lengths), "\n")
wo_na <- results_df[!is.na(results_df$telomere_end), ]
cat("Mean telomere length at 20-10 without NA: ", mean(wo_na$telomere_length))
cat("\n")
l <- which(results_df$telomere_end == "L")
r <- which(results_df$telomere_end == "R")
cat("Fraction of L telomeres: ", length(l) / nrow(results_df), "\n")
cat("Fraction of R telomeres: ", length(r) / nrow(results_df), "\n")
...
telomeres_fl <- list.files(file.path(data_dir, "telo"), 
                           full.names = TRUE)
ref_file <- file.path(paths$example_data, "ref", "chm13v2.0.fa.gz")
...
top <- ggplot(left, aes(x = ref_name, y = telomere_length, fill = chromEnd)) +
  geom_violin(drop = FALSE) +
  geom_boxplot(width = 0.1) +
  scale_fill_manual(values = c("#0091ff")) +
  labs(x = "Chromosome", 
    y = "Telomere Length", 
    title = "Telomere Lengths by Reference Name 5'") +
    theme(axis.text.x = element_text(angle=90, vjust=.5, hjust=1))

bottom <- ggplot(right, aes(x = ref_name, y = telomere_length, fill = chromEnd)) +
  geom_violin(drop = FALSE) +
  geom_boxplot(width = 0.1) +
  scale_fill_manual(values = c("#ff00b7")) +
  labs(x = "Chromosome", 
    y = "Telomere Length", 
    title = "Telomere Lengths by Reference Name 3'") +
  theme(axis.text.x = element_text(angle=90, vjust=.5, hjust=1)) 

grid <- plot_grid(top, bottom, ncol = 1)
save_plot(file.path(paths$example_results, sample_name, "violin_plot.png"), grid, base_width = 16, base_height = 8)
save_plot(file.path(paths$example_results, sample_name, "histogram.png"), hist, base_width = 12, base_height = 8)
```