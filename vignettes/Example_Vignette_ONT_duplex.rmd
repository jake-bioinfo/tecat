...
```{r source}
#source("00-paths.R")
source(file.path("R", "fG-helper_functions.R"))
library(Biostrings)
...
sample_file <- file.path("/home/jake/tmp/comparative_study_temp/HG00732/lr/ERR4987503-5.fastq")
data_dir <- file.path("/home/jake/tmp/comparative_study_temp/tmp")
results_dir <- file.path("/home/jake/tmp/comparative_study_temp/tmp/results")
...
source(file.path("R", "f01-Fastq_Processing.R"))
...
if(!file.exists(list.files(file.path(dirname(sample_file), "split"), 
  ...
  # Save stats
  saveRDS(stats_fl, file.path(results_dir, 
                                "stats_fl.rds"))
}
```
...
source(file.path("R", "f02-Reference_Processing.R"))
options(meme_bin = "~/meme/bin")
reference_file <- file.path("/home/jake/science_projects/telo/shared_data/hs/ref/chm13v2.0.fa.gz")
...
# ONT Settings
motif_analysis <- telomere_motif(reference_file = reference_file,
                                 reference_telo_length = 2000,
                                 telo_motif_length = 6, 
                                 number_of_motifs = 10,
...
motif_analysis <- readRDS(file.path(results_dir,
                                    "motif_analysis_results.rds"))
                                    
# Source functions for pattern searching
source(file.path("R", "f03-Pattern_searching.R"))
...
search_res <- telo_search(fastq_files = file_list, 
...
# Save telomere_stats
saveRDS(search_res, file.path(results_dir,
                                  "telomere_stats.rds"))
```

## Cut telomere reads into sliding windows
```{r telo_cut, eval = TRUE}
telomere_stats <- readRDS(file.path(results_dir, 
                                    "telomere_stats.rds"))

telo_file <- list.files(file.path(data_dir, "telo"),
                        pattern = "combined",
                        full.names = TRUE)

# Source functions for cutting telomere reads into sliding windows
source(file.path("R", "f04-Sliding_window.R"))

# Run sliding window on all telomere files
windows <- sliding_window_parallel(telomere_file = telo_file, 
                                       window_length = 200,
                                       step = 100, 
                                       environment = "linux",
                                       threads = 18,
...
# Save windows                                       
saveRDS(windows, file.path(results_dir, 
                           "windows.rds"))
# Satus
...
source(file.path("R", "f05-Generate_telomere_frequencies.R"))
...
# Directories
win_file <- file.path(results_dir,
        "windows.rds")
...
out_dir = file.path(results_dir, 
                    "frequencies")
# Test
freqs <- frequencies(windows = wins, 
                     motifs = motifs, 
                     environment = "linux", 
                     parallel = TRUE, 
                     threads = 18, 
...
# Save Frequencies
saveRDS(freqs, file.path(results_dir, 
                         "freqs.rds"))
...
# Load data
telomere_frequencies <- readRDS(file.path(results_dir,
                                          "freqs.rds"))

# Source functions for determining telomere length
source(file.path("R", "f06-Telomere_length.R"))
...
# Save threshold data
saveRDS(thresh_df, file.path(results_dir,
                             "threshold_data.rds"))
```
...
telomere_frequencies <- readRDS(file.path(results_dir,
                                          "freqs.rds"))
thresh_df <- readRDS(file.path(results_dir,
                               "threshold_data.rds"))
...
print(paste0("Mean of telomere lengths at 50-50 which were less than 2 kbp: ", 
        mean(less_2k), "\n"))
# breaks <- ifelse(length(less_2k) < 10, 1, sqrt(length(less_2k)))
# hist(less_2k, breaks = breaks, 
...
results <- invisible(parallel::parLapply(cl, telomere_frequencies, 
                                         findTelLength, st.thresh = thresholds$sensitivity, 
...
# Save results
saveRDS(results_df, file.path(results_dir, 
                              "results_df.rds"))
...
source(file.path("R", "fG-helper_functions.R"))
source(file.path("R", "f07-Truncate_Map.R"))
# Load data
results_df <- readRDS(file.path(results_dir, 
                                "results_df.rds"))
...
telomeres_file <- list.files(file.path(data_dir, "telo"), 
...
# Truncate files
fasta_fl <- truncate_file(combined_telomere_file = telomeres_file, 
...
# Save mapped output
saveRDS(mapped_output, file.path(results_dir, 
                                 "mapped_output.rds"))
```
...
# Load data
mapped_output <- readRDS(file.path(results_dir,
                                   "mapped_output.rds"))
